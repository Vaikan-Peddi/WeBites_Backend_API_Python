...
def select_college(college_id: int, curr_user: schemas.UserInDB = Depends(util.get_current_user), db: Session = Depends(db.get_db)):
...
def update_college(college_id: int, curr_user: schemas.UserInDB = Depends(util.get_current_user), db: Session = Depends(db.get_db)):
